...
import io.agriledger.model.LarvaeTestingModel
import io.agriledger.model.api.FruitFlowDTO
import io.agriledger.model.api.FruitFlowStagesDTO
import io.agriledger.model.api.LarvaeTestingDTO
...
object LarvaeTestingFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: LarvaeTestingDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
//                                larvaeTestingData = reqParam?.larvaeTesting?.let {
                                LarvaeTestingModel(
                                        larvaeTesting = if (reqParam?.larvaeTesting != null) {
                                            reqParam.larvaeTesting
                                        } else {
                                            null
                                        },
                                        rejectReason = if (reqParam?.rejectReason != null) {
                                            reqParam.rejectReason
                                        } else {
                                            null
                                        }
                                )
//                                }
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowLarvaeTesting(), listOf(ourIdentity.owningKey))
