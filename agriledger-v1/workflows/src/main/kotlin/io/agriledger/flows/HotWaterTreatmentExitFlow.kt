...
object HotWaterTreatmentExitFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: HotWaterTreatmentExitDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                hotWaterTreatmentExitData = reqParam?.let {
                                    HotWaterTreatmentExitModel(
                                            durationHotWaterTreatment = reqParam.durationHotWaterTreatment,
                                            timeofExitHotWaterTreatment = if (reqParam.timeofExitHotWaterTreatment != null) {
                                                LocalDateTime.parse(reqParam.timeofExitHotWaterTreatment)
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowHotWaterTreatmentExit(), listOf(ourIdentity.owningKey))
