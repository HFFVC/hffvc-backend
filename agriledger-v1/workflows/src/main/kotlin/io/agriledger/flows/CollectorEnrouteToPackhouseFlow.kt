...
import io.agriledger.model.EnrouteToPackhouseModel
import io.agriledger.model.api.EnrouteToPackhouseDTO
...
object CollectorEnrouteToPackhouseFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val reqParam: EnrouteToPackhouseDTO, val email: String?) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    enrouteToPackhouseData = EnrouteToPackhouseModel(
                            enroutePackhouseAddedOn = LocalDateTime.parse(reqParam.enroutePackhouseAddedOn),
                            enroutePackhouseAdditionalNote = reqParam.enroutePackhouseAdditionalNote,
            ...
            val command = Command(ServiceRequestContract.Commands.CollectorEnrouteToPackhouse(), listOf(ourIdentity.owningKey))
        ...
        private fun validateFlowConstraints(reqParam: EnrouteToPackhouseDTO): Boolean {
            if (
                    reqParam.dropOffAtPackhouse.isBlank() ||
                    reqParam.enroutePackhouseAddedOn.isBlank()
            ) {
                return false
            }
            return true
