...
object ColdRoomStorageInFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: ColdRoomStorageInDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                coldRoomStorageInData = reqParam?.let {
                                    ColdRoomStorageInModel(
                                            fruitTemperatureColdStorage = if (reqParam.fruitTemperatureColdStorage != null) {
                                                reqParam.fruitTemperatureColdStorage?.toFloat()
                                            } else {
                                                null
                                            },
                                            airflowTemperatureRHColdStorage = if (reqParam.airflowTemperatureRHColdStorage != null) {
                                                reqParam.airflowTemperatureRHColdStorage?.toFloat()
                                            } else {
                                                null
                                            },
                                            storageTimeIn = if (reqParam.storageTimeIn != null) {
                                                LocalDateTime.parse(reqParam.storageTimeIn)
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruiFlowColdRoomStorageIn(), listOf(ourIdentity.owningKey))
