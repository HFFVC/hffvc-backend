...
import io.agriledger.model.api.FruitWashingDTO
...
object FruitWashingFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: FruitWashingDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                fruitWashingData = reqParam?.let {
                                    FruitWashingModel(
                                            waterTemperature = if (reqParam.waterTemperature != null) {
                                                reqParam.waterTemperature?.toFloat()
                                            } else {
                                                null
                                            },
                                            phLevel = if (reqParam.phLevel != null) {
                                                reqParam.phLevel?.toFloat()
                                            } else {
                                                null
                                            },
                                            chlorineLevel = if (reqParam.chlorineLevel != null) {
                                                reqParam.chlorineLevel?.toFloat()
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowFruitWashing(), listOf(ourIdentity.owningKey))
