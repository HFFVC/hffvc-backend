...
import io.agriledger.model.EnrouteToProducerModel
import io.agriledger.model.api.EnrouteToProducerDTO
...
object CollectorEnRouteToProducerFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val reqParam: EnrouteToProducerDTO, val email: String?) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    enrouteToProducerData = EnrouteToProducerModel(
                            plannedPickUpAddedOn = LocalDateTime.parse(reqParam.plannedPickUpAddedOn),
                            enrouteAdditionalNote = reqParam.enrouteAdditionalNote
                    ),
            ...
            val command = Command(ServiceRequestContract.Commands.CollectorEnRouteToProducer(), listOf(ourIdentity.owningKey))
        ...
        private fun validateFlowConstraints(reqParam: EnrouteToProducerDTO): Boolean {
            if (
                    reqParam.plannedPickUpAddedOn.isBlank() ||
                    reqParam.status.isBlank()
            ) {
                return false
            }
            return true
