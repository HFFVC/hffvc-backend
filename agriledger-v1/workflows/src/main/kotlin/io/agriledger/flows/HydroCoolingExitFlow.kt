...
object HydroCoolingExitFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: HydroCoolingExitDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                hydroCoolingExitData = reqParam?.let {
                                    HydroCoolingExitModel(
                                            durationHydroCooling = reqParam.durationHydroCooling,
                                            internalFruitTempHydroCooling = if (reqParam.internalFruitTempHydroCooling != null) {
                                                reqParam.internalFruitTempHydroCooling?.toFloat()
                                            } else {
                                                null
                                            },
                                            timeofExitHydroCooling = if (reqParam.timeofExitHydroCooling != null) {
                                                LocalDateTime.parse(reqParam.timeofExitHydroCooling)
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowHydroCoolingExit(), listOf(ourIdentity.owningKey))
