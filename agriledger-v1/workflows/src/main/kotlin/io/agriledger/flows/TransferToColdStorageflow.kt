...
object TransferToColdStorageflow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: TransferToColdStorageDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                transferTocoldStorageData = reqParam?.let {
                                    TransferToColdStorageModel(
                                            temperatureTransfer = if (reqParam.temperatureTransfer != null) {
                                                reqParam.temperatureTransfer?.toFloat()
                                            } else {
                                                null
                                            },
                                            coldStorageTimeIn = if (reqParam.coldStorageTimeIn != null) {
                                                LocalDateTime.parse(reqParam.coldStorageTimeIn)
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowTransferToColdStorage(), listOf(ourIdentity.owningKey))
