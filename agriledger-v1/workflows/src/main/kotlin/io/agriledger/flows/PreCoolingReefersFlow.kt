...
object PreCoolingReefersFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: PreCoolingRefersDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                preCoolingReefersData = reqParam?.let {
                                    PreCoolingReefersModel(reeferWallTemperature = if (reqParam.reeferWallTemperature != null) {
                                        reqParam.reeferWallTemperature?.toFloat()
                                    } else {
                                        null
                                    })
                                }
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowPreCoolingReefers(), listOf(ourIdentity.owningKey))
