...
import io.agriledger.model.api.QualityInspectionDTO
...
object QualityInspectionFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: QualityInspectionDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                qualityInspectionData = reqParam?.let {
                                    QualityInspectionModel(
                                            fruitsAccepted = if (reqParam.fruitsAccepted != null) {
                                                reqParam.fruitsAccepted?.toInt()
                                            } else {
                                                null
                                            },
                                            fruitsRejected = if (reqParam.fruitsRejected != null) {
                                                reqParam.fruitsRejected?.toInt()
                                            } else {
                                                null
                                            },
                                            qualityInspectionResults = reqParam.qualityInspectionResults
                                    )
                                }
                        )
                    } else {
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowQualityInspection(), listOf(ourIdentity.owningKey))
