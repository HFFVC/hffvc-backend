...
import io.agriledger.model.api.UnloadingAtBuyerPlaceDTO
...
object UnloadingAtBuyerFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val saleId: String, val reqParam: UnloadingAtBuyerPlaceDTO?, val email: String?) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: SalesState = vaultData.copy(
                    unloadingAtBuyerData = reqParam?.let {
                        UnloadingAtBuyerModel(
                                dateAndTimeOfUnLoading = LocalDateTime.parse(reqParam.dateAndTimeOfUnLoading),
                                totalNoofBoxesUnLoaded = reqParam.totalNoofBoxesUnLoaded.toInt()
                        )
                    },
            ...
            val command = Command(SalesContract.SalesCommand.UnloadingAtBuyer(), listOf(ourIdentity.owningKey))
