...
object RemovalFromColdStorageFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: RemovalFromColdStorageDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                removalFromColdStorageData = reqParam?.let {
                                    RemovalFromColdStorageModel(
                                            temperatureRemoval = if (reqParam.temperatureRemoval != null) {
                                                reqParam.temperatureRemoval?.toFloat()
                                            } else {
                                                null
                                            },
                                            coldStorageTimeOut = if (reqParam.coldStorageTimeOut != null) {
                                                LocalDateTime.parse(reqParam.coldStorageTimeOut)
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowRemovalFromColdStorage(), listOf(ourIdentity.owningKey))
