...
object SecondSizingFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: SecondSizingDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                secondSizingData = reqParam?.let {
                                    SecondSizingModel(secondSizingResults = reqParam.secondSizingResults)
                                }
                        )
                    } else {
...
//                    creationData = vaultData.creationData.copy(status = reqParam.status),
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowSecondSizing(), listOf(ourIdentity.owningKey))
