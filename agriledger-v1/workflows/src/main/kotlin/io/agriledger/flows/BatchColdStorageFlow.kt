...
// *********
    ...
    class Initiator(val id: String
                    , val reqParam: BatchColdStorageDTO?, val email: String?) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: BatchState = vaultData.copy(
                    batchUpdateData = if (vaultData.batchUpdateData == null) {
                        UpdateBatchModel(
                                coldStorageData = reqParam?.let {
                                    BatchColdStorageModel(
                                            temperature = reqParam.temperature.toFloat(),
                                            phLevel = reqParam.phLevel.toFloat(),
                                            co2Level = reqParam.co2Level.toFloat(),
                                            ethyleneLevel = reqParam.ethyleneLevel.toFloat(),
                                            coldStorageInTimestamp = LocalDateTime.parse(reqParam.coldStorageInTimestamp)
                                    )
                                }
                        )
                    } else {
            ...
            val command = Command(BatchContract.BatchCommands.BatchColdStorage(), listOf(ourIdentity.owningKey))
