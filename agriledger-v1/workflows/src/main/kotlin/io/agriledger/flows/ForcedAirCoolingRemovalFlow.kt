...
object ForcedAirCoolingRemovalFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: ForcedAirCoolingRemovalDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                forcedAirCoolingRemovalData = reqParam?.let {
                                    ForcedAirCoolingRemovalModel(
                                            removalTimeForcedCooling = if (reqParam.removalTimeForcedCooling != null) {
                                                LocalDateTime.parse(reqParam.removalTimeForcedCooling)
                                            } else {
                                                null
                                            },
                                            fruitTemperatureRemovalForcedCooling = if (reqParam.fruitTemperatureRemovalForcedCooling != null) {
                                                reqParam.fruitTemperatureRemovalForcedCooling?.toFloat()
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowForcedAirCoolingRemoval(), listOf(ourIdentity.owningKey))
