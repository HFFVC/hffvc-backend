...
// *********
    ...
    class Initiator(
        val id: String,
        val reqParam: ShippingDetailsDTO?,
        val email: String?,
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                fruitFlowData = if (vaultData.fruitFlowData == null) {
                    FruitFlowModel(
                        shippingDetailsData = reqParam?.let {
                            ShippingDetailsModel(
                                shippingDetails = if (reqParam.shippingDetails != null) {
                                    Shipping.valueOf(reqParam.shippingDetails!!)
                                } else {
                                    null
                                }
                            )
            ...
            val command =
                Command(ServiceRequestContract.Commands.FruitFlowShippingDetails(), listOf(ourIdentity.owningKey))
