...
object DeleteServiceRequestFlow {

    @InitiatingFlow
    @StartableByRPC
    class Initiator(val serviceRequestId: String): FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val wantedStateRef = QueryVault().queryServiceRequestById(serviceRequestId, Vault.StateStatus.UNCONSUMED, serviceHub.vaultService)
                    ?: throw FlowException("Service Request $serviceRequestId not found.")

            progressTracker.currentStep = TX_COMPONENTS
            val command = Command(ServiceRequestContract.Commands.DeleteServiceRequest(), listOf(ourIdentity.owningKey))
