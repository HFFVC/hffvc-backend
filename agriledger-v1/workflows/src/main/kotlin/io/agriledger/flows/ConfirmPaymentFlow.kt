...
object ConfirmPaymentFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val saleId: String, val reqParam: ConfirmPaymentDTO?, val email: String?) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: SalesState = vaultData.copy(
                    confirmPaymentData = reqParam?.let {
                        ConfirmPaymentModel(
                                confirmPaymentDate = LocalDateTime.parse(reqParam.confirmPaymentDate),
                                netReceivables = reqParam.netReceivables.toFloat(),
                                paymentNetReceivablesCurrency = reqParam.paymentNetReceivablesCurrency,
                                wasFactored = reqParam.wasFactored.toBoolean(),
                                amountFactored = reqParam.amountFactored?.toFloat(),
                                factoringEntity = reqParam.factoringEntity,
                                factoringAgent = reqParam.factoringAgent,
                                factoringContactDetails = reqParam.factoringContactDetails,
                                factoringCharges = reqParam.factoringCharges?.toFloat(),
                                escrowAccountNo = reqParam.escrowAccountNo,
                                brokerBankTransactionCurrency = reqParam.brokerBankTransactionCurrency,
                                brokerBankTransactionFee = reqParam.brokerBankTransactionFee.toFloat(),
                                netPayable = reqParam.netPayable.toFloat()
                        )
                    },
            ...
            val command = Command(SalesContract.SalesCommand.ConfirmPayment(), listOf(ourIdentity.owningKey))
