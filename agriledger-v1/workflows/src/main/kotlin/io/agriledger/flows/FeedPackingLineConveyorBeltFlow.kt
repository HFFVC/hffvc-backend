...
object FeedPackingLineConveyorBeltFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: FeedPackingLineConveyorBeltDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                feedPackingLineConveyorData = reqParam?.let {
                                    FeedPackingLineConveyorBeltModel(packingLineTimeofEntry = if (reqParam.packingLineTimeofEntry != null) {
                                        LocalDateTime.parse(reqParam.packingLineTimeofEntry)
                                    } else {
                                        null
                                    })
                                }
            ...
            val command = Command(ServiceRequestContract.Commands.FruiFlowFeedPackingLineConveyorBelt(), listOf(ourIdentity.owningKey))
