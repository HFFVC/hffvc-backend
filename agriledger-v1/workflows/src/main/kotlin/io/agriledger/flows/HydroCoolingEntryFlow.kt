...
object HydroCoolingEntryFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: HydroCoolingEntryDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                hydroCoolingEntryData = reqParam?.let {
                                    HydroCoolingEntryModel(
                                            timeofEntryHydroCooling = if (reqParam.timeofEntryHydroCooling != null) {
                                                LocalDateTime.parse(reqParam.timeofEntryHydroCooling)
                                            } else {
                                                null
                                            },
                                            chlorineLevelHydroCooling = if (reqParam.chlorineLevelHydroCooling != null) {
                                                reqParam.chlorineLevelHydroCooling?.toFloat()
                                            } else {
                                                null
                                            },
                                            phLevelHydroCooling = if (reqParam.phLevelHydroCooling != null) {
                                                reqParam.phLevelHydroCooling?.toFloat()
                                            } else {
                                                null
                                            },
                                            waterTemperatureHydroCooling = if (reqParam.waterTemperatureHydroCooling != null) {
                                                reqParam.waterTemperatureHydroCooling?.toFloat()
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowHydroCoolingEntry(), listOf(ourIdentity.owningKey))
