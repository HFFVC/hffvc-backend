...
object ColdTunnelLoadingFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: ColdTunnelLoadingDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
...
//
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                coldTunnelLoadingData = reqParam?.let {
                                    ColdTunnelLoadingModel(
                                            destinationDateTimeColdTunnelLoading = if (reqParam.destinationDateTimeColdTunnelLoading != null) {
                                                LocalDateTime.parse(reqParam.destinationDateTimeColdTunnelLoading)
                                            } else {
                                                null
                                            },
                                            reeferWallTemperatureColdTunnelLoading = if (reqParam.reeferWallTemperatureColdTunnelLoading != null) {
                                                reqParam.reeferWallTemperatureColdTunnelLoading?.toFloat()
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowColdTunnelLoading(), listOf(ourIdentity.owningKey))
