...
import io.agriledger.model.CleaningAndTrimmingModel
import io.agriledger.model.FruitFlowModel
import io.agriledger.model.api.CleaningAndTrimmingDTO
...
object CleaningAndTrimmingFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: CleaningAndTrimmingDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                cleaningAndTrimmingData = reqParam?.let {
                                    CleaningAndTrimmingModel(
                                            dateTimeCleaningTrimming = if (reqParam.dateTimeCleaningTrimming != null) {
                                                LocalDateTime.parse(reqParam.dateTimeCleaningTrimming)
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowCleaningAndTrimming(), listOf(ourIdentity.owningKey))
