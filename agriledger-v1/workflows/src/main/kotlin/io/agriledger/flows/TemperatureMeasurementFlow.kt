...
import io.agriledger.model.FruitFlowModel
import io.agriledger.model.TemperatureMeasurementModel
import io.agriledger.model.api.FruitFlowDTO
import io.agriledger.model.api.TemperatureMeasurementDTO
...
object TemperatureMeasurementFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: TemperatureMeasurementDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                temperatureMeasurementData = reqParam?.let {
                                    TemperatureMeasurementModel(
                                            internalFruitTemp = if (reqParam.internalFruitTemp != null) {
                                                reqParam.internalFruitTemp?.toFloat()
                                            } else {
                                                null
                                            },
                                            ambientTemp = if (reqParam.ambientTemp != null) {
                                                reqParam.ambientTemp?.toFloat()
                                            } else {
                                                null
                                            },
                                            isTemepertaureBreach = if (reqParam.isTemepertaureBreach != null) {
                                                reqParam.isTemepertaureBreach?.toBoolean()
                                            } else {
                                                null
                                            },
                                            temperatureBreachCount = if (reqParam.temperatureBreachCount != null) {
                                                reqParam.temperatureBreachCount?.toInt()
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowTemperatureMeasurement(), listOf(ourIdentity.owningKey))
