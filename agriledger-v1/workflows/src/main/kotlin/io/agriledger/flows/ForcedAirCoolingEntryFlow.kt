...
object ForcedAirCoolingEntryFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: ForcedAirCoolingEntryDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                forcedAirCoolingEntryData = reqParam?.let {
                                    ForcedAirCoolingEntryModel(
                                            entryTimeForcedCooling = if (reqParam.entryTimeForcedCooling != null) {
                                                LocalDateTime.parse(reqParam.entryTimeForcedCooling)
                                            } else {
                                                null
                                            },
                                            fruitTemperatureEntryForcedCooling = if (reqParam.fruitTemperatureEntryForcedCooling != null) {
                                                reqParam.fruitTemperatureEntryForcedCooling?.toFloat()
                                            } else {
                                                null
                                            },
                                            airflowTemperatureRH = if (reqParam.airflowTemperatureRH != null) {
                                                reqParam.airflowTemperatureRH?.toFloat()
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowForcedAirCoolingEntry(), listOf(ourIdentity.owningKey))
