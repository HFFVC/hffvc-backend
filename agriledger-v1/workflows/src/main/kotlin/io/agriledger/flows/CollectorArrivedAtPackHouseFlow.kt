...
import io.agriledger.model.ArrivedAtPackhouseModel
import io.agriledger.model.api.ArrivedAtPackHouseDTO
...
object CollectorArrivedAtPackHouseFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val reqParam: ArrivedAtPackHouseDTO, val email: String?) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    arrivedAtPackhouseData = ArrivedAtPackhouseModel(
                            arrivedAtPackhouseAddedOn = LocalDateTime.parse(reqParam.arrivedAtPackhouseAddedOn)
                            , arrivedAtPackhouseAdditionalNote = reqParam.arrivedAtPackhouseAdditionalNote
                            , transportCostArrivedAtPackhouse = reqParam.transportCostArrivedAtPackhouse?.toFloat()
                            , timeOfAdmittance = LocalDateTime.parse(reqParam.timeOfAdmittance)
                    ),
            ...
            val command = Command(ServiceRequestContract.Commands.CollectorArrivedAtPackHouse(), listOf(ourIdentity.owningKey))
        ...
        private fun validateFlowConstraints(reqParam: ArrivedAtPackHouseDTO): Boolean {
            if (
                    reqParam.arrivedAtPackhouseAddedOn.isBlank() ||
                    reqParam.timeOfAdmittance.isBlank() ||
                    (reqParam.transportCostArrivedAtPackhouse != null && (
                            reqParam.transportCostArrivedAtPackhouse!!.isBlank() ||
                                    reqParam.transportCostArrivedAtPackhouse!!.toFloat() < 0
                            )
                            )
