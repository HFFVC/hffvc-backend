...
object UpdateNoSaleStatusFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val batchId: String, val email: String?) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val wantedStateRef = QueryVault().queryBatchById(batchId, Vault.StateStatus.UNCONSUMED, serviceHub.vaultService)
            if (wantedStateRef == null) {
                throw Exception("Batch ID does not exist.")
            }

            // getting all service request ids
            val allWantedStateRefs = QueryVault().queryServiceRequestByBatch(batchId, Vault.StateStatus.UNCONSUMED, serviceHub.vaultService)
            ...
            val command = Command(ServiceRequestContract.Commands.UpdateNoSale(), listOf(ourIdentity.owningKey))
