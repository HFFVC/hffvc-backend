...
object TransportDetailsFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: TransportDetailsDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val _status: String = reqParam?.status ?: "PACKING COMPLETED"

            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                transportDetailsData = reqParam?.let {
                                    TransportDetailsModel(
                                            departureDateTimeTransport = if (reqParam.departureDateTimeTransport != null) {
                                                LocalDateTime.parse(reqParam.departureDateTimeTransport)
                                            } else {
                                                null
                                            },
                                            destinationTransport = reqParam.destinationTransport,
                                            fruitFlowCompletedOn = if (reqParam.fruitFlowCompletedOn != null) {
                                                LocalDateTime.parse(reqParam.fruitFlowCompletedOn)
                                            } else {
                                                null
                                            },
                                            lossOfBox = if (reqParam.lossOfBox != null) {
                                                reqParam.lossOfBox?.toInt()
                                            } else {
                                                null
                                            },
                                            penalityAmount = if (reqParam.penalityAmount != null) {
                                                reqParam.penalityAmount?.toFloat()
                                            } else {
                                                null
                                            },
                                            transportConditions = reqParam.transportConditions,
                                            transportCost = if (reqParam.transportCost != null) {
                                                reqParam.transportCost?.toFloat()
                                            } else {
                                                null
                                            },
                                            transportCurrency = reqParam.transportCurrency,
                                            transportTemperature = if (reqParam.transportTemperature != null) {
                                                reqParam.transportTemperature?.toFloat()
                                            } else {
                                                null
                                            }
                                    )
                    ...
                    creationData = vaultData.creationData.copy(status = _status),
                    signatories = newSignatories,
                    usernames = usernames
            )

            val command = Command(ServiceRequestContract.Commands.FruitFlowTransportDetails(), listOf(ourIdentity.owningKey))
