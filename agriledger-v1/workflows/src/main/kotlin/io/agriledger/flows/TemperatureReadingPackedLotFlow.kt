...
object TemperatureReadingPackedLotFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: TemperatureReadingPackedLotDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                temperatureReadingPackedLotData = reqParam?.let {
                                    TemperatureReadingPackedLotModel(
                                            ambienttemperaturePacked = if (reqParam.ambienttemperaturePacked != null) {
                                                reqParam.ambienttemperaturePacked?.toFloat()
                                            } else {
                                                null
                                            },
                                            internalFruitTemperaturePacked = if (reqParam.internalFruitTemperaturePacked != null) {
                                                reqParam.internalFruitTemperaturePacked?.toFloat()
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowTemperatureReadingPackedLot(), listOf(ourIdentity.owningKey))
