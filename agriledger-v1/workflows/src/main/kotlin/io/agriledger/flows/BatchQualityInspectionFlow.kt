...
object BatchQualityInspectionFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    ...
    class Initiator(val id: String
                    , val reqParam: BatchQualityInspectionDTO?, val email: String?) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
                        ...
                        UpdateBatchModel(
                                qualityInspectionData = reqParam?.let {
                                    BatchQualityInspectionModel(
                                            fruitsRejected = reqParam.fruitsRejected.toInt(),
                                            additionalComments = reqParam.additionalComments
                                    )
                                }
                        )
                    } else {
            ...
            val command = Command(BatchContract.BatchCommands.BatchQualityInspection(), listOf(ourIdentity.owningKey))

            // We create a transaction buiclder and add the components.
