...
object FirstSizingFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: FirstSizingDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                firstSizingData = reqParam?.let {
                                    FirstSizingModel(firstSizingResults = if (reqParam.firstSizingResults != null) {
                                        reqParam.firstSizingResults
                                    } else {
                                        null
                                    })
                                }
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowFirstSizing(), listOf(ourIdentity.owningKey))
