...
object DeletePenaltiesFlow {
    // *********
    // * Flows *
    // *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val wantedLotStateRef = QueryVault().queryServiceRequestById(id, Vault.StateStatus.UNCONSUMED, serviceHub.vaultService)
            ...
            val lotOutputState: ServiceRequestState = lotVaultData.copy(
                    penaltiesIncurred = mutableListOf<PenaltyDataModel>()
            )

            progressTracker.currentStep = TX_COMPONENTS

            val serviceRequestCommand = Command(ServiceRequestContract.Commands.DeletePenalties(), listOf(ourIdentity.owningKey))
