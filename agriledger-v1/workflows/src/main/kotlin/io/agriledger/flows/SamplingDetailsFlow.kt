...
object SamplingDetailsFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: SamplingDetailsDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                samplingDetailsData = reqParam?.let {
                                    SamplingDetailsModel(
                                            samplesTaken = if (reqParam.samplesTaken != null) {
                                                reqParam.samplesTaken?.toInt()
                                            } else {
                                                null
                                            },
                                            samplingTemperature = if (reqParam.samplingTemperature != null) {
                                                reqParam.samplingTemperature?.toFloat()
                                            } else {
                                                null
                                            },
                                            dateAndTimeofSampling = if (reqParam.dateAndTimeofSampling != null) {
                                                LocalDateTime.parse(reqParam.dateAndTimeofSampling)
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowSampling(), listOf(ourIdentity.owningKey))
