...
import io.agriledger.model.GradingModel
import io.agriledger.model.api.FruitFlowDTO
import io.agriledger.model.api.GradingDTO
...
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: GradingDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
        ...
        override fun call(): SignedTransaction {
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                gradingData = reqParam?.let {
                                    GradingModel(
                                            gradingResults = reqParam.gradingResults,
                                            weightofRemovedFruit = if (reqParam.weightofRemovedFruit != null) {
                                                reqParam.weightofRemovedFruit?.toFloat()
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowGrading(), listOf(ourIdentity.owningKey))
