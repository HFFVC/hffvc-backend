...
object ColdRoomStorageOutFlow {
    // *********
// * Flows *
// *********
    @InitiatingFlow
    @StartableByRPC
    class Initiator(val id: String, val reqParam: ColdRoomStorageOutDTO?, val email: String?, val fruitFlow: FruitFlowDTO) : FlowLogic<SignedTransaction>() {
        companion object {
...
//
            ...
            val outputState: ServiceRequestState = vaultData.copy(
                    fruitFlowData = if (vaultData.fruitFlowData == null) {
                        FruitFlowModel(
                                coldRoomStorageOutData = reqParam?.let {
                                    ColdRoomStorageOutModel(
                                            fruitTemperatureColdStorageOut = if (reqParam.fruitTemperatureColdStorageOut != null) {
                                                reqParam.fruitTemperatureColdStorageOut?.toFloat()
                                            } else {
                                                null
                                            },
                                            storageTimeOut = if (reqParam.storageTimeOut != null) {
                                                LocalDateTime.parse(reqParam.storageTimeOut)
                                            } else {
                                                null
                                            }
                                    )
            ...
            val command = Command(ServiceRequestContract.Commands.FruitFlowColdRoomStorageOut(), listOf(ourIdentity.owningKey))
